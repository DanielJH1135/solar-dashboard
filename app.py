...
app = Flask(__name__)# -*- coding: utf-8 -*-
import os
import requests
import xml.etree.ElementTree as ET
from flask import Flask, request, jsonify
...
# 2. Vercel 진입점 단일 노출 (Vercel 배포 시 app.run 블록 제외)
app = Flask(__name__)
...
<body class="p-4 md:p-6 max-w-7xl mx-auto">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 border-b border-gray-800 pb-5">
        <div>
            ...
            <p class="text-xs md:text-sm text-gray-400 mt-1">VWorld & 국토부 연동 (2.5평/선납 일시금 마스터 버전)</p>
        </div>
    </header>
            ...
            <details class="bg-gray-900 border border-gray-800 rounded-2xl shadow-xl group" open>
                ...
                <div class="p-5 flex flex-col gap-4">
                    ...
                    <div class="bg-gradient-to-b from-blue-500/10 to-transparent border-2 border-blue-500/40 rounded-xl p-4 relative">
                        ...
                        <div id="promoWarning" class="mb-3 text-center text-[10px] text-rose-400 font-semibold bg-rose-950/20 p-1.5 rounded border border-rose-900/30 hidden">
                            ⚠️ 선택된 임대사의 최소 진입 용량 미달로 임대가 불가능합니다.
                        </div>

                        <div id="rentalPanel" class="flex flex-col gap-2">
                            <div class="flex justify-between items-center bg-gray-950 p-2 rounded border border-gray-850 text-xs">
                                <span class="text-gray-500">초기 지주 투자비용</span>
                                <span class="font-bold text-blue-400">0원</span>
                            </div>
                            
                            <div class="flex justify-between items-center bg-blue-950/30 p-2.5 rounded border border-blue-900/50 text-xs">
                                <span class="text-blue-300 font-bold" id="rentLabel1">계약 시 선납 일시금</span>
                                <span class="font-black text-amber-400 text-sm"><span id="rentInitialTotal">0</span> 원</span>
                            </div>
                            
                            <div class="flex justify-between items-center bg-gray-950/50 p-1.5 rounded text-[10px]">
                                <span class="text-gray-500 pl-2">↳ <span id="rentLabel1_sub">N년치 임대료 선납분</span></span>
                                <span class="font-bold text-gray-400"><span id="rentInitialBase">0</span> 원</span>
                            </div>
                            <div class="flex justify-between items-center bg-gray-950/50 p-1.5 rounded text-[10px]">
                                <span class="text-gray-500 pl-2">↳ 기간한정 프로모션 합산</span>
                                <span class="font-bold text-gray-400"><span id="rentInitialPromo">0</span> 원</span>
                            </div>

                            <div class="flex justify-between items-center bg-gray-950 p-2 rounded border border-gray-850 text-xs mt-1">
    ...
    <script>
        ...
        function calculateValues() {
            ...
            // 🚨 선납 일시금 + 프로모션 정밀 수식 반영
            if (kw > 0 && kw >= activeBrand.minKw) {
                document.getElementById('rentalPanel').classList.remove('hidden');
                document.getElementById('promoWarning').classList.add('hidden');

                const annualRent1 = kw * activeBrand.rate1;
                const initialBaseTotal = annualRent1 * activeBrand.y1;
                const promoTotal = kw * activeBrand.promo;
                const grandInitialTotal = initialBaseTotal + promoTotal;
                
                const annualRent2 = kw * activeBrand.rate2;

                document.getElementById('rentInitialTotal').innerText = Math.round(grandInitialTotal).toLocaleString();
                document.getElementById('rentInitialBase').innerText = Math.round(initialBaseTotal).toLocaleString();
                document.getElementById('rentInitialPromo').innerText = Math.round(promoTotal).toLocaleString();
                document.getElementById('rentAnnual2').innerText = Math.round(annualRent2).toLocaleString();

                document.getElementById('rentLabel1').innerText = "계약 일시금 (" + activeBrand.y1 + "년치 선납 + 프로모션)";
                document.getElementById('rentLabel1_sub').innerText = activeBrand.y1 + "년치 임대료 선납분";
                document.getElementById('rentLabel2').innerText = "후기 연 임대료 (" + activeBrand.y2 + "년간 매년 지급)";
            } else {
                document.getElementById('rentalPanel').classList.add('hidden');
